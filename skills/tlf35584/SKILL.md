...
use_cases:
  ...
  - "配置 VMON 电压监控通道过压/欠压阈值及故障响应策略"
  - "配置看门狗窗口时间（WDW1/WDW2）并实现窗口/问答两种触发序列"
  - "实现 TLF35584 安全状态机（INIT/NORMAL/SLEEP/WAKE/FAILSAFE）转换逻辑"
  - "读取 TLF35584 故障诊断寄存器（DEVSTAT/MONSF/OTFAIL）并上报 Dem 故障事件"
...
```yaml
knowledge_areas:
  - primary-area: "TLF35584 PMIC 技术"
    topics:
      - "TLF35584 内部架构（LDO/DCDC/VMON/WDG/安全状态机模块划分）"
      - "TLF35584 SPI 帧格式（16 位帧：地址 6 位/R-W 位/数据 8 位/奇校验 1 位）"
      - "TLF35584 寄存器映射（DEVCFG/SYSPCFG/WDCFG/VMONSTAT/DEVSTAT 等全集）"
      - "TLF35584 状态机转换（INIT→NORMAL→SLEEP→WAKE→FAILSAFE 路径与触发条件）"
      - "TLF35584 看门狗机制（窗口模式/问答模式，WDW1/WDW2 时间配置与触发序列）"
      - "TLF35584 VMON 监控通道（UV/OV 阈值精度、故障响应：ERR 引脚/FAILSAFE 触发）"

  - secondary-area: "PMIC 功能安全集成"
    topics:
      - "TLF35584 与 MCU 安全联动（SAFE 引脚/ERR 引脚/RST 引脚行为定义）"
      - "TLF35584 在 ISO 26262 系统中的 ASIL-D 安全机制角色（Hardware Safety Element）"
      - "PMIC 电源序列时序（上电顺序/掉电顺序/电压稳定等待时间要求）"
      - "TLF35584 故障诊断寄存器解析（MONSF/OTFAIL/INITERR 故障位映射与 Dem 上报）"
      - "AUTOSAR EcuM 与 TLF35584 状态管理集成（Sleep/Wake 驱动接口对接）"
```
...
段落 A：Approach（执行步骤）
  ...
  1. 查询 knowledge/tlf35584-datasheet.md（寄存器定义、SPI 帧时序与电源轨参数）
  2. 评审电源树设计文档，确认各电源轨目标电压与 VMON 阈值要求
  3. 实现 TLF35584 SPI 16 位帧读写函数（含奇校验计算，统计 bit15-bit1 全部 15 位）
...
段落 B：Standards & Best Practices（规范遵循）
  ...
  遵循英飞凌 TLF35584 Application Note（官方应用手册规定的寄存器初始化顺序）
...
段落 C：Deliverables（交付物定义）
  ...
  每次执行必须输出：
  驱动源码：Tlf35584Drv.c / Tlf35584Drv.h，含完整 Doxygen 注释（@asil/@brief/@param/@return）
  寄存器定义：Tlf35584_RegMap.h，含全部寄存器地址与位字段宏定义
  配置文件：Tlf35584Drv_Cfg.h，含 WDG 窗口时间与 VMON 阈值可配置参数
  单元测试：Test_Tlf35584Drv_<Feature>.c，100% MC/DC 覆盖（ASIL-D 强制）
...
段落 D：Safety & Security Considerations（安全合规检查）
  ...
  ✋ HUMAN CHECK：所有 TLF35584 WDG/VMON/SafeState 配置变更必须由功能安全工程师书面确认
...
```yaml
examples:
  - prompt: "实现 TLF35584 SPI 16 位帧写操作，包含奇校验位计算"
    response: |
      ## 分析说明
      ...
      ## 代码片段
      ```c
      /* Tlf35584Drv.c - SPI 16位帧写操作（含奇校验） */
      ...
      - 奇校验计算：统计 bit15-bit1 共 15 位，正确遵循 TLF35584 Application Note 规范
      - SPI 同步传输：使用 Spi_SyncTransmit 确保写入完成后才返回，无异步竞争风险

      ## 建议
      - 写操作后对关键配置寄存器添加读回验证（Read-Back Verification）
      - 在多任务环境中通过 SchM_Enter_Tlf35584Drv 保护 SPI 总线访问的临界区
```
...
```yaml
constraints:
  - "标准合规：TLF35584 驱动代码必须符合 MISRA-C:2012，安全关键代码零豁免"
  - "安全等级：TLF35584 为 ASIL-D PMIC，所有 WDG/VMON/SafeState 变更必须触发 HUMAN CHECK"
...
integration_points:
  - system: "TLF35584 PMIC 硬件"
    ...
    protocol: "TLF35584 SPI 通信协议（英飞凌 TLF35584 Application Note）"
  - system: "MCU Safety Pack"
    interface: "SAFE/ERR/RST 引脚"
    protocol: "TLF35584-MCU 安全联动接口（SAFE 引脚低电平 → MCU 安全复位触发）"
  - system: "AUTOSAR EcuM"
    interface: "软件接口"
    protocol: "EcuM 电源状态管理（调用 TLF35584 Sleep/Wake 驱动接口）"
...
performance_criteria:
  - metric: "执行时间"
    target: "< 45 分钟完成 TLF35584 完整驱动标准实现（含注释与单元测试框架）"
  - metric: "首次质量"
...
```yaml
human_checks:
  - condition: "任何 TLF35584 WDG 窗口时间（WDW1/WDW2）配置变更"
  ...
  - condition: "TLF35584 安全状态机（FAILSAFE 进入/退出条件）逻辑修改"
  ...
  - condition: "TLF35584 驱动被定义为 ASIL-D 安全关键组件的唯一实现路径，无独立评审"
  ...
  - condition: "遇到未记录在 TLF35584 Application Note 中的寄存器行为或异常时序"
