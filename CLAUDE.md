...
```
第1层：MASTER_INDEX.md          ← 先读这里，定位目标分类（~800 tokens）
第2层：{section}/{category}/INDEX.md  ← 再读分类索引，找到具体文件（~300 tokens）
第3层：具体 .md 文件            ← 最后按需加载（~400 tokens/文件）
```
### 强制规则

| 场景 | 正确做法 | 禁止操作 |
|------|----------|----------|
| 查找某个知识点 | 读 `MASTER_INDEX.md` → 定位 → 读具体文件 | ❌ 批量读取整个目录 |
| 了解某分类 | 读 `skills/autosar/INDEX.md` | ❌ `glob("skills/autosar/*.md")` |
| 查询所有 Agent | 读 `agents/*/INDEX.md`（按需） | ❌ 一次加载全部 36 个 agent |
| 不确定在哪 | 读 `MASTER_INDEX.md` 导航 | ❌ 递归搜索整个 skills/ |
